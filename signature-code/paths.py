...
def fBm_paths(grid_points, M, H, T):
    """
    This function generates M trajectories of the process $W^H = \int_0^t (t-s)^{H-0.5} dW_s$ with Cholesky decomposition.
...
def generate_rBergomi_paths(n_paths_train, n_paths_eval, grid_points, Hs, T, etas, V_0):
    def generate_rBergomi(n_paths, grid_points, Hs, etas):
        ...
        for i in range(n_paths):
            Y[i, 0] = random.choice(Hs)
            X[i, :] = rBergomi_paths(grid_points, M=1, H=Y[i, 0], T=T, eta=etas, V_0=V_0)
            print(f'已生成第 {i + 1} 条路径...')
        return X, Y
    print('******开始生成训练集******')
...
def generate_rHeston_paths(n_paths_train, n_paths_eval, grid_points, T, V_0, reps, Hs, kappa_1s, kappa2s, thetas):
    def generate_rHeston(n_paths, grid_points, Hs, kappa1s, kappa2s, thetas):
        ...
        for i in range(n_paths):
            Y[i, 0] = random.choice(Hs)
            X[i, :] = rHeston_paths(grid_points, M=1, H=Y[i, 0], T=T, kappa_1=kappa1s, kappa_2=kappa2s, theta=thetas, V_0=V_0, reps=reps)
            print(f'已生成第 {i + 1} 条路径...')
        return X, Y
    print('******开始生成训练集******')
...
if __name__ == "__main__":
    ...
    kappa1s = 0.1; kappa2s = 0.03; thetas = 0.3
    V_0 = 0.01; reps = 1e-6
    etas = 1
    grid_points = 1000; T = 1
    n_paths_train = 3000; n_paths_eval = 1000
    X_train, Y_train, X_eval, Y_eval = generate_fBm_paths(n_paths_train, n_paths_eval, grid_points, Hs_uniform, T)
    np.save('simulated_data/fBm/Xtrain_grid=1000_Huniform.npy', X_train)
    np.save('simulated_data/fBm/Ytrain_grid=1000_Huniform.npy', Y_train)
    np.save('simulated_data/fBm/Xeval_grid=1000_Huniform.npy', X_eval)
    np.save('simulated_data/fBm/Yeval_grid=1000_Huniform.npy', Y_eval)
