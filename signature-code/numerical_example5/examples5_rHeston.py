...
'''导入数据集'''
X_train = np.load('data/simulated_data/rHeston/Xtrain_grid=500_Hbeta_kappa1uniform_kappa2uniform_thetauniform_v0=0.01.npy')
Y_train = np.load('data/simulated_data/rHeston/Ytrain_grid=500_Hbeta_kappa1uniform_kappa2uniform_thetauniform_v0=0.01.npy')
...
torch.save(transformer, 'data/results/multiple_rHeston/trained_models/transformer.pth')
print('******Transformer训练完成******')

print('******开始训练SigFormer******')
sigformer = models_rHeston.sigformer(augment_include_original=True, augment_include_time=True, T=1)
model_trainer(sigformer, 'SigFormer', history)
torch.save(sigformer, 'data/results/multiple_rHeston/trained_models/sigformer.pth')
print('******SigFormer训练完成******')
...
torch.save(deepsignet, 'data/results/multiple_rHeston/trained_models/deepsignet.pth')
print('******DeepSigNet训练完成******')

print('******开始训练CNN******')
cnn = models_rHeston.cnn()
model_trainer(cnn, 'CNN', history)
torch.save(cnn, 'data/results/multiple_rHeston/trained_models/cnn.pth')
print('******CNN训练完成******')
...
plt.savefig('data/results/multiple_rHeston/rHeston_grid=500_Hbeta_kappa1uniform_kappa2uniform_thetauniform_v0=0.01_round1.eps', bbox_inches='tight')
