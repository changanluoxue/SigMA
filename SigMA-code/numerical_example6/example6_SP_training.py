...
torch.save(deepsignet, 'data/results/trained_models/deepsignet.pth')
print('******DeepSigNet训练完成******')

print('******开始训练CNN******')
cnn = models.cnn()
model_trainer(cnn, 'CNN', history)
torch.save(cnn, 'data/results/trained_models/cnn.pth')
print('******CNN训练完成******')
...
torch.save(transformer, 'data/results/trained_models/transformer.pth')
print('******Transformer训练完成******')

'''输出训练结果'''
params = {}
for i, j in zip(('LSTM', 'SigMA', 'DeepSigNet', 'CNN', 'Transformer'),
                (lstm, sigma, deepsignet, cnn, transformer)):
...
for k in ('LSTM', 'Transformer', 'SigMA', 'DeepSigNet', 'CNN'):
