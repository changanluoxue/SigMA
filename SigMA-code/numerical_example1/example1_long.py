...
model_trainer(sigma_stridehalf, 'SigMA_stridehalf', history)
torch.save(sigma_stridehalf, f'data/results/input_length_{input_length}/{volatility_model}/trained_models/{H_distribution}/round{round}/sigma_stridehalf.pth')
print('******SigMA_stridehalf训练完成******')

'''输出训练结果'''
params = {}
for i, j in zip((r'$\rm SigMA_{stride=50}$', 'SigMA_stridehalf'),
...
for k in (r'$\rm SigMA_{stride=50}$', 'SigMA_stridehalf'):
