...
class sigma(nn.Module):
    def __init__(self, augment_include_original=True, augment_include_time=True, T=1):
        ...
        self.linear_layer.weight.data = torch.tensor([[1,0,0,0], [0,3,0,0], [0,0,5,0], [0,0,0,1]], dtype=torch.float32)
...
class transformer(nn.Module):
    def __init__(self, augment_include_original=True, augment_include_time=True, T=1):
        ...
        self.dense = fnn(input_size=77500, hidden_sizes=(32, 32, 32, 32, 32), output_size=4, activation=nn.ReLU())
        self.sigmoid = nn.Sigmoid()
        self.flatten = nn.Flatten(start_dim=1, end_dim=-1)
...
class lstm(nn.Module):
    def __init__(self):
        ...
        '(time_grids, input_size): (500, 63872)'
        self.dense = fnn(input_size=63872, hidden_sizes=(128, 64), output_size=4, activation=nn.PReLU())
        self.flatten = nn.Flatten(start_dim=1, end_dim=-1)
