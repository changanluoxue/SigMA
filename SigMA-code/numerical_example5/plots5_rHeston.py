...
total_errors = {'Transformer': [], 'SigMA': [], 'Deepsignet': [], 'CNN': [], 'LSTM': []}
total_merrors = {'Transformer': [], 'SigMA': [], 'Deepsignet': [], 'CNN': [], 'LSTM': []}
RMSEs = {'Transformer': [], 'SigMA': [], 'Deepsignet': [], 'CNN': [], 'LSTM': []}

for round_num in range(1, 11):
    # models loaded
    transformer = torch.load(f'data/results/multiple_rHeston/trained_models/round{round_num}/transformer.pth')
    sigma = torch.load(f'data/results/multiple_rHeston/trained_models/round{round_num}/sigma.pth')
    deepsignet = torch.load(f'data/results/multiple_rHeston/trained_models/round{round_num}/deepsignet.pth')
    cnn = torch.load(f'data/results/multiple_rHeston/trained_models/round{round_num}/cnn.pth')
    lstm = torch.load(f'data/results/multiple_rHeston/trained_models/round{round_num}/lstm.pth')
    # models evaluation
    transformer.eval()
    sigma.eval()
    deepsignet.eval()
    ...
    models = {'Transformer': transformer, 'SigMA': sigma,
              'Deepsignet': deepsignet, 'CNN': cnn, 'LSTM': lstm}
    # prediction
...
box_data = [total_merrors['Transformer'], total_merrors['SigMA'],
            total_merrors['Deepsignet'], total_merrors['CNN'], total_merrors['LSTM']]
box_data_H = [total_errors['Transformer'][:,0], total_errors['SigMA'][:,0],
              total_errors['Deepsignet'][:,0], total_errors['CNN'][:,0], total_errors['LSTM'][:,0]]
box_data_kappa1 = [total_errors['Transformer'][:,1], total_errors['SigMA'][:,1],
                total_errors['Deepsignet'][:,1], total_errors['CNN'][:,1], total_errors['LSTM'][:,1]]
box_data_kappa2 = [total_errors['Transformer'][:,2], total_errors['SigMA'][:,2],
                total_errors['Deepsignet'][:,2], total_errors['CNN'][:,2], total_errors['LSTM'][:,2]]
box_data_theta = [total_errors['Transformer'][:,3], total_errors['SigMA'][:,3],
                total_errors['Deepsignet'][:,3], total_errors['CNN'][:,3], total_errors['LSTM'][:,3]]
labels = ['Transformer', 'SigMA', 'Deepsignet', 'CNN', 'LSTM']
