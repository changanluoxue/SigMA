...
class sigma(nn.Module):
    def __init__(self, augment_include_original=True, augment_include_time=True, T=1):
        ...
        # set time_grid=500, then (truncation order, input_size): (1,10),(3,310),(5,7810)
    ...
    def forward(self, x):
        ...
        if self.augment_include_time is True:
            x = torch.cat((x, time), dim=1)
        x = torch.transpose(x, 1, 2)
        # truncation order 1,3,5 (stride equals half of the input length)
        x = torch.stack([layers.Sig(x[:, :250*i, :], 5) for i in range(1, 1+int(x.size(1)/250))])
