...
# example_batch_x: (batch, feature, length); example_batch_y: (batch, feature)
...
torch.save(lstm, f'data/results/input_length_{input_length}/{volatility_model}/trained_models/{H_distribution}/round{round}/LSTM.pth')
print('******LSTM训练完成******')
...
torch.save(transformer, f'data/results/input_length_{input_length}/{volatility_model}/trained_models/{H_distribution}/round{round}/Transformer.pth')
...
torch.save(sigma, f'data/results/input_length_{input_length}/{volatility_model}/trained_models/{H_distribution}/round{round}/SigMA.pth')
print('******SigMA训练完成******')
...
torch.save(deepsignet, f'data/results/input_length_{input_length}/{volatility_model}/trained_models/{H_distribution}/round{round}/DeepSigNet.pth')
print('******DeepSigNet训练完成******')

print('******开始训练CNN******')
cnn = models.cnn()
model_trainer(cnn, 'CNN', history)
torch.save(cnn, f'data/results/input_length_{input_length}/{volatility_model}/trained_models/{H_distribution}/round{round}/CNN.pth')
print('******CNN训练完成******')

'''输出训练结果'''
params = {}
for i, j in zip(('SigMA', 'DeepSigNet', 'CNN', 'LSTM', 'Transformer'),
                (sigma, deepsignet, cnn, lstm, transformer)):
...
for k in ('SigMA', 'DeepSigNet', 'CNN', 'LSTM', 'Transformer'):
