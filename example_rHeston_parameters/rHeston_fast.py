...
def generate_rHeston(n_paths, n_samples, kappas, alphas):
	"""Generate FBM paths"""
	X = np.zeros((n_paths, n_samples+1))
	y = np.zeros((n_paths, 2))
	for j in range(n_paths):
		y[j,0] = random.choice(kappas); y[j,1] = random.choice(alphas)
		X[j,:] = rHfast(dt=1/n_samples, M=1, N=n_samples, s0=100, v0=0, rf=0, kappa=y[j,0], theta=0.3, epsilon=0.3, rho=-0.7, alpha=y[j,1], reps=1e-6)
	return X, y
def generate_data(n_paths_train, n_paths_test, n_samples, kappas, alphas):
	"""Generate train and test datasets"""

	# generate dataset
	x_train, y_train = generate_rHeston(n_paths_train, n_samples, kappas, alphas)
	x_test, y_test = generate_rHeston(n_paths_test, n_samples, kappas, alphas)
