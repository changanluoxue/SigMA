...
def rBergomi_path_chol(grid_points, M, hurst_exponents, T):
    """
    ...
    for j in range(grid_points-1):
        for i in range(grid_points-1):
            if i==j:
                Sigma[i,j]= np.power(X[i], 2 * hurst_exponents) / 2 / hurst_exponents
            else:
                s=np.minimum(X[i],X[j])
                t=np.maximum(X[i],X[j])
                Sigma[i,j]= np.power(t - s, hurst_exponents - 0.5) / (hurst_exponents + 0.5) * np.power(s, 0.5 + hurst_exponents) * special.hyp2f1(0.5 - hurst_exponents, 0.5 + hurst_exponents, 1.5 + hurst_exponents, -s / (t - s))
    ...
    return W*np.power(T, hurst_exponents)
...
def generate_rBergomi(n_paths, n_samples, hurst_exponents):
	"""Generate FBM paths"""
	X = np.zeros((n_paths, n_samples+1))
	y = np.zeros(n_paths)
	for j in range(n_paths):
		y[j] = random.choice(hurst_exponents)
		X[j,:] = rBergomi_path_chol(grid_points=n_samples+1, M=1, hurst_exponents=y[j], T=1)
		# X[j,:] = np.log(X[j,:]/X[j,0])
	return X, y
def generate_data(n_paths_train, n_paths_test, n_samples, hurst_exponents):
	"""Generate train and test datasets"""

	# generate dataset
	x_train, y_train = generate_rBergomi(n_paths_train, n_samples, hurst_exponents)
	x_test, y_test = generate_rBergomi(n_paths_test, n_samples, hurst_exponents)
