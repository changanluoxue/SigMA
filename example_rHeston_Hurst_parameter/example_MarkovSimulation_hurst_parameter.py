...
import rHeston_MarkovSimulation
...
hurst_exponents=np.around(np.linspace(0.01, 0.2, 7), decimals=2).tolist()
...
history = {}
x_train, y_train, x_test, y_test = rHeston_MarkovSimulation.generate_data(n_paths_train, n_paths_test, n_samples, hurst_exponents=hurst_exponents)
x_train_, x_test_ = rHeston_MarkovSimulation.preprocess_data(x_train, x_test)
(train_dataloader, test_dataloader,
 example_batch_x, example_batch_y) = rHeston_MarkovSimulation.generate_torch_batched_data(x_train_, y_train, x_test_, y_test,
...
# RNN
rnn = rHeston_MarkovSimulation.deep_recurrent(output_shape,
                                     sig=False,
                                     augment_layer_sizes=(),
...
# DeeperSigNet
deepersignet = rHeston_MarkovSimulation.deep_recurrent(output_shape,
                                              sig=True,
                                              sig_depth=sig_depth_deepAdeeper,
...
# generate dataset
x_train_, x_test_ = rHeston_MarkovSimulation.preprocess_data(x_train, x_test, flag='lstm')

# generate torch dataloaders
(train_dataloader_lstm, test_dataloader_lstm,
 example_batch_lstm_x, example_batch_lstm_y) = rHeston_MarkovSimulation.generate_torch_batched_data(x_train_, y_train, x_test_, y_test,
                                                                                           train_batch_size, val_batch_size)
# trainer function
...
# LSTM
lstmnet = rHeston_MarkovSimulation.LSTM(input_dim=1,
                               num_layers=2,
                               hidden_dim=32,
                               output_dim=1,
                               final_nonlinearity=torch.sigmoid)
...
# GRU
grunet = rHeston_MarkovSimulation.GRU(input_dim=1,
                             num_layers=2,
                             hidden_dim=32,
                             output_dim=1,
                             final_nonlinearity=torch.sigmoid)
...
# generate dataset
x_train_, x_test_ = rHeston_MarkovSimulation.preprocess_data(x_train, x_test, flag='neuralsig')

# generate torch dataloaders
(train_dataloader_sig, test_dataloader_sig,
 example_batch_sig_x, example_batch_sig_y) = rHeston_MarkovSimulation.generate_torch_batched_data(x_train_, y_train,
                                                                                         x_test_, y_test,
                                                                                         train_batch_size, val_batch_size)
