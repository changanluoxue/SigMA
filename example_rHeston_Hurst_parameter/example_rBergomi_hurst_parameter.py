...
history = {}
x_train, y_train, x_test, y_test = rBergomi.generate_data(n_paths_train, n_paths_test,
                                                                 n_samples, hurst_exponents)


x_train_, x_test_ = rBergomi.preprocess_data(x_train, x_test)

(train_dataloader, test_dataloader,
 example_batch_x, example_batch_y) = rBergomi.generate_torch_batched_data(x_train_, y_train,
                                                                                 x_test_, y_test,
...
rnn = rBergomi.deep_recurrent(output_shape,
                                     sig=False,
                                     augment_layer_sizes=(),
...
deepersignet = rBergomi.deep_recurrent(output_shape,
                                              sig=True,
                                              sig_depth=sig_order,
...
x_train_, x_test_ = rBergomi.preprocess_data(x_train, x_test, flag='lstm')

(train_dataloader_lstm, test_dataloader_lstm,
 example_batch_lstm_x, example_batch_lstm_y) = rBergomi.generate_torch_batched_data(x_train_,
                                                                                           y_train,
                                                                                           x_test_,
                                                                                           y_test,
                                                                                           train_batch_size,
...
lstmnet = rBergomi.LSTM(input_dim=1,
                               num_layers=2,
                               hidden_dim=32,
                               output_dim=1,
                               final_nonlinearity=torch.sigmoid)
...
grunet = rBergomi.GRU(input_dim=1,
                             num_layers=2,
                             hidden_dim=32,
                             output_dim=1,
                             final_nonlinearity=torch.sigmoid)
...
# generate dataset
x_train_, x_test_ = rBergomi.preprocess_data(x_train, x_test, flag='neuralsig')

# generate torch dataloaders
(train_dataloader_sig, test_dataloader_sig,
 example_batch_sig_x, example_batch_sig_y) = rBergomi.generate_torch_batched_data(x_train_,
                                                                                         y_train,
                                                                                         x_test_,
                                                                                         y_test,
                                                                                         train_batch_size,
...
plt.savefig('C:\CodeFiles\PythonFiles\PythonProjects\signature_transformer_2023\example_rHeston_Hurst_parameter\data\example_hurst_parameter.eps'
            , bbox_inches='tight')
